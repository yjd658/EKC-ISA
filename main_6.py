...
from model import SentimentClassifier_v8,report_metrics,visualize_feature_space,similarity_loss3
...
from parameters import print_params_stat

torch.manual_seed(2021)
torch.cuda.empty_cache()
train_data = read_data('train.txt','K_train.pt',lanaugue='zh')
dev_data = read_data('dev.txt','K_dev.pt',lanaugue='zh')
test_data = read_data('test.txt','K_test.pt',lanaugue='zh')
...
model = SentimentClassifier_v8().to(device)
...
for epoch in range(num_epochs):
    ...
    for name, para in model.named_parameters():
        model.state_dict()[name][:] += (torch.rand(para.size(), device=device) - 0.5) * 0.25 * torch.std(para)
    total_loss = 0.0
    total_preds = []
    total_labels = []
    total_class_sums = torch.zeros(3, 768, device=device)
    ...
    for batch in train_loader:
        ...
        loss3,total_class_counts=similarity_loss3(conbintation, labels,weight, N,total_class_sums)
        # loss2, total_class_sums, total_class_counts = similarity_loss2(conbintation, labels, total_class_sums,total_class_counts)
